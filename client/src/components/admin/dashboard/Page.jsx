...
import {
  getRequestData as getRequestDataService,
  getUserDistribution as getUserDistributionService,
  getMoneyData as getMoneyDataService,
} from "@/services/analysis";
import { useSelector } from "react-redux";

const COLORS = ["#4f46e5", "#10b981", "#f59e0b", "#f44336"];

export default function DashboardPage() {
  const [range, setRange] = useState("day");
  const { token } = useSelector((state) => state.auth);
  const [requestDataDay, setRequestDataDay] = useState([
  ...
  const [requestDataMonth, setRequestDataMonth] = useState([
  ...
  const [requestDataYear, setRequestDataYear] = useState([
  ...
  useEffect(() => {
    const getUserDistribution = async () => {
      await getUserDistributionService({ token: token, setPieData });
    };
    getUserDistribution();
  }, [token]);
  ...
  useEffect(() => {
    const getMoneyData = async () => {
      await getMoneyDataService({ token: token, setMoneyData });
    };
    getMoneyData();
  }, [token]);
