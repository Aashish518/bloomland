...
exports.approveOne = async (req, res) => {
  try {
    ...
    // Send WhatsApp message on approval
    try {
      // const whatsappResponse = await sendMessage(
      //   `whatsapp:${request.userId.phone}`,
      //   "approved"
      // );
...
exports.rejectOne = async (req, res) => {
  try {
    ...
    // Remove the Approved document if it exists for this user and event
    await Approved.findOneAndDelete({
      userId: request.userId._id,
      eventId: request.eventId._id,
    });
    ...
    // Send WhatsApp message on rejected
    try {
      // const whatsappResponse = await sendMessage(
      //   `whatsapp:${request.userId.phone}`,
      //   "rejected"
...
exports.enterOne = async (req, res) => {
  try {
    ...
    if (joined.status !== "pending") {
      return res.status(400).json({
        message: "Joinee either cancelled or entered already",
      });
    }
...
exports.deleteApproval = async (req, res) => {
  try {
    ...
    if (approved.status !== "pending") {
      return res.status(400).json({
        message: "Approved user paid already",
      });
    }
...
exports.addAttendee = async (req, res) => {
  try {
    ...
    const existingAttendee = Joined.findOne({
      userId: user._id,
      eventId: eventId,
    });
    if (existingAttendee) {
    ...
    const attendee = await Joined.create({
      eventId: eventId,
      userId: user._id,
    });
    ...
    return res.status(500).json({
      message: "Error while adding attendee",
    });
  }
};
